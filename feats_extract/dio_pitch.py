...
from feats_extract.tools import pad_list
from feats_extract.abs_feats_extract import AbsFeatsExtract
...
class Dio(AbsFeatsExtract):
    ...
    def forward(
        ...
        pitch_max_per_phoneme = [
            self._extract_max_value_by_duration(p, d).view(-1)
            for p, d in zip(pitch, durations)
        ]

        # Padding
        pitch_per_phoneme = pad_list(pitch_per_phoneme, 0.0)
        pitch_max_per_phoneme = pad_list(pitch_max_per_phoneme, 0.0)

        pitch_cwt = self.cwt_extract(pitch_per_phoneme)

        # Return with the shape (B, T, 1)

        return pitch[0], pitch_per_phoneme.unsqueeze(-1), pitch_lengths, pitch_cwt, pitch_max_per_phoneme.unsqueeze(-1)

    def cwt_extract(self, pitch):
        pitch_cwt_dict = dict()
    ...
    def _average_by_duration(self, x: torch.Tensor, d: torch.Tensor) -> torch.Tensor:
        if not 0 <= len(x) - d.sum() < self.reduction_factor:
            d[-1] += len(x) - d.sum()
    ...
    def _extract_max_value_by_duration(self, x:torch.tensor, d: torch.Tensor) -> torch.Tensor:
        if not 0 <= len(x) - d.sum() < self.reduction_factor:
            d[-1] += len(x) - d.sum()
        d_cumsum = F.pad(d.cumsum(dim=0), (1, 0))
        x_max = [
            torch.max(x[start:end].masked_select(x[start:end].gt(0.0)))
            if len(x[start:end].masked_select(x[start:end].gt(0.0))) != 0
            else x.new_tensor(0.0)
            for start, end in zip(d_cumsum[:-1], d_cumsum[1:])
        ]
        return torch.stack(x_max)
