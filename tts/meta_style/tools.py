...
def get_mask_from_lengths(lengths, max_len=None, is_inference=False):
    if is_inference:
        batch_size = 1

        ids = torch.arange(0, max_len).unsqueeze(0).expand(batch_size, -1).to('cpu')
        ...
        logging.info(f'ids: {ids.shape}')
        mask = ids >= lengths.unsqueeze(1).expand(-1, max_len)
        logging.info(f'mask_shape: {mask.shape}')
